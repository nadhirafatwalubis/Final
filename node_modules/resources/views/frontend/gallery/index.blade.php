@extends('frontend.layouts.app')

@section('content')
<section class="hero-wrap hero-wrap-2 js-fullheight"
    style="background-image: url('{{ config('settings.bg_header_other_url') }}')">
    <div class="overlay"></div>
    <div class="container">
        <div class="row no-gutters slider-text js-fullheight align-items-end justify-content-center">
            <div class="col-md-9 ftco-animate breadcrumb-wrapper text-center">
                <p class="breadcrumbs">
                    <span class="mr-2">
                        <a href="{{ url('/') }}">Home <i class="fa fa-chevron-right"></i></a>
                    </span>
                    <span>Gallery<i class="fa fa-chevron-right"></i>
                    </span>
                </p>
                <h1 class="mb-0 bread">Gallery</h1>
            </div>
        </div>
    </div>
</section>
@include('frontend.layouts.booking')
<section class="ftco-section">
    <div class="container">
        <div class="row">
            @foreach ($galleries as $gallery)
            <div class="col-md-4 ftco-animate">
                <div class="project-wrap">
                    <a href="{{ route('gallery.show', $gallery->slug) }}" class="img"
                        style="background-image: url('{{ $gallery->cover_thumb_url}}');">
                    </a>
                    <div class="text p-4">
                        <h3>
                            <a href="{{ route('gallery.show', $gallery->slug) }}">
                                {{ $gallery->title }}
                            </a>
                        </h3>
                    </div>
                </div>
            </div>
            @endforeach
        </div>
        <div class="row mt-5">
            <div class="col text-center">

            </div>
        </div>
        <div class="row mt-5">
            <div class="col text-center">
                {{ $galleries->links('pagination::frontend') }}
            </div>
        </div>
    </div>
    </div>
</section>
@endsection